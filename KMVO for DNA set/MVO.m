...
function [newDNA]=MVO(N,Max_time,lb,ub,dim,Length,H_target,count)
...
%Universes=initialization(N,dim,ub,lb);%N 随机的一列数
    Universes=InitDNASet(Length,H_target,count);
    HammingDist = DistHammingMatrix(Universes);
    n = size(Universes,1); 
%Universes=InitDNASet(Length,H_target,count);
%HammingDist = DistHammingMatrix(Universes); 
...
%         end
         ...
         b=find(temp==a);
       for i=1:length(B)
        DisH=Hamming(Universes(B(i),:),newDNA);
...
%          end
    ...
    for i=2:size(Universes,1)%Starting from 2 since the firt one is the elite
        ...
        for j=1:size(Universes,2)
            ...
            if   GCContent(Universes(i,:))~=1||Nolength(Universes(i,:))==1
                nDNA=[];
    ...
    for i=1:size(Universes,1)
        DisH=Hamming(Universes(i,:),newDNA);
        GCH=GCContent(Universes(i,:));
        len1=find(DisH<H_target);
       if isempty(len1)
