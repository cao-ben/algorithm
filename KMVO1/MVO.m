...
%            if j== dim
%                 
%                 Universes(i,a)=rand()*(ub-lb);
%            end
        end
    end
%          if Time>=Max_time*8/10
%              BiUniverses=[];
%             %交叉
%             Universes=exchange(Universes,1);
%             %k-means聚类
%             c=randperm(size(Universes,1));
%             cen= cat(1,Best_universe,Universes(c(1),:));
%             [GUniverses,BUniverses]  = k_means(Universes, cen,2);
%              BiUniverses=exchange(GUniverses,1);
%              l=abs(size(BUniverses,1)-size(GUniverses,1));
%              BiUniverses=cat(1,BiUniverses,Universes(1:l,:));
%             Universes=cat(1,GUniverses,BiUniverses);
            %存在问题：聚类完了少了一半宇宙
%          end
          %if Time>=Max_time*29/100 && Time<=Max_time*30/100
          if Time<=Max_time*5/100
            Universes=exchange(Universes,1);
              %k-means聚类
               %cen= cat(1,Best_universe,SD);
            cen= cat(1,Best_universe,Sorted_universes(size(Universes,1),:));
            [GUniverses,BUniverses]  = k_means(Universes, cen,2);
             %交叉
            BUniverses=exchange(BUniverses,1);
            Universes=cat(1,GUniverses,BUniverses);
            %存在问题：聚类完了少了一半宇宙
          else
              Universes=exchange(Universes,1);
         end
...
%              %交叉
%             BiUniverses=exchange(GUniverses,1);
%             l=size(BUniverses,1)-size(GUniverses,1);
%             BiUniverses=cat(1,BiUniverses,BUniverses(1:l,:));
%             Universes=cat(1,GUniverses,BiUniverses);
%             %存在问题：聚类完了少了一半宇宙
%          end
...
%     SD=[SD Best_universe_Inflation_rate];
